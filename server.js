...
ableToMeetApp.use('/js', express.static(__dirname + '/public/js'));
ableToMeetApp.use('/css', express.static(__dirname + '/public/css'));
ableToMeetApp.use('/audio', express.static(__dirname + '/public/audio'));
ableToMeetApp.use('/images', express.static(__dirname + '/public/images'));

// Add a route for telemetry scripts
if (debugMode) {
    ableToMeetApp.use('/telemetry', express.static(__dirname + '/public/telemetry'));
}

// Set up main index page (this changes depending on whether or not debugging is enabled in settings.json).
ableToMeetApp.get('/', function(req, res) {
...
easyrtc.listen(ableToMeetApp, socketServer);
